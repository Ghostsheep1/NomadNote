...
export function HomeDashboard({ trips, onCreateTrip, onImportTrip }: HomeDashboardProps) {
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl border border-border/80 bg-card shadow-sm sm:rounded-[28px]">
      <div className="absolute inset-0 travel-hero-surface" />
      <div className="relative grid gap-5 p-4 sm:gap-6 sm:p-7 lg:grid-cols-[1.2fr_0.8fr] lg:p-8">
        <div className="min-w-0">
          <div className="mb-4 inline-flex max-w-full items-center gap-2 rounded-full border border-primary/15 bg-background/70 px-3 py-1 text-xs font-semibold text-primary shadow-sm backdrop-blur sm:mb-5">
            <ShieldCheck className="h-3.5 w-3.5" />
            <span className="truncate">Local-first travel planning by Henrique Ribeiro</span>
          </div>
          <h1 className="max-w-2xl font-display text-[2.5rem] font-bold leading-[0.9] tracking-normal sm:text-5xl md:text-6xl">
...
function MetricCard({
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-background/75 p-3 shadow-sm backdrop-blur sm:p-4">
      <div
        className={cn(
